define([""], function(){
    ...
    methods.buildSummary = function(dbData, selectedCrypto) {
        var lastData = dbData.cryptoSavings[dbData.cryptoSavings.length - 1];
        var summary = "\
        <table class=\"table\">\
          ...
          <tbody>\
              <tr>\
                  ...
                  <td>"+lastData.cryptoAmount+"</td>\
                  <td id=\"latest-price\">$"+Number(dbData.latestPrice).formatMoney(2)+"</td>\
                  <td  id=\"latest-value\"><strong>$"+(dbData.latestPrice * Number(lastData.cryptoAmount) - Number(lastData.spent)).formatMoney(2)+"</strong></td>\
              </tr>\
          </tbody>\
        </table>";
    ...
    methods.buildTable = function(dbData) {
        var table = "\
        ...
        dbData.cryptoSavings.forEach(function(obj) {
            var tableRow = "\
            <tr>\
                <td>"+obj.date+"</td>\
                <td>$"+Number(obj.spent).formatMoney(2)+"</td>\
                <td>"+obj.cryptoAmount+"</td>\
                <td>$"+Number(obj.avgPrice).formatMoney(2)+"</td>\
                <td>$"+Number(obj.cryptoValue).formatMoney(2)+"</td>\
            </tr>";
            table += tableRow;
        });
        var latestObj = dbData.cryptoSavings[dbData.cryptoSavings.length - 1];
        table += "\
          <tr>\
            <td>"+new Date().toDateString().substr(4)+"</td>\
            <td>$"+Number(latestObj.spent).formatMoney(2)+"</td>\
            <td>"+Number(latestObj.cryptoAmount)+"</td>\
            <td>$"+dbData.latestPrice.formatMoney(2)+"</td>\
            <td>$"+(Number(latestObj.cryptoAmount) * dbData.latestPrice).formatMoney(2)+"</td>\
          </tr>\
        </tbody>\
        </table>";
    ...
    methods.buildChartData = function(dbData) {
        var data = {
          type:'line',
          data:{
            labels:[],
        ...
        dbData.cryptoSavings.forEach(function(obj) {
            ...
            data.data.datasets[0].data.push(Number(obj.cryptoValue));
        });
        console.log(data.data.datasets[0].data);
        data.data.datasets[0].label = (dbData.userData.crypto).toUpperCase();
        data.data.labels.push(new Date().toDateString());
        data.data.datasets[0].data.push((Number(dbData.cryptoSavings[dbData.cryptoSavings.length - 1].cryptoAmount) * dbData.latestPrice).toFixed(2));
    ...
    methods.printOldest = function(selectedCrypto) {
      var url = this.buildUrl(selectedCrypto);
